...
switch prevStep
    ...
    case 'convert'
        prevStep = 'tc_BP1_trans_*.mat';
    case 'ICA_artifacts'
        prevStep = 'Mtc_BP1_trans_*.mat';
    case 'ICA_artifacts_copy'
        prevStep = 'Mtc_BP1_trans_*.mat';
    case 'downsample'
        prevStep = 'd*Mtc_BP1_trans_*.mat';
    case 'epoch'
        prevStep = 'e*Mtc_BP1_trans_*.mat';
    case 'merge'
        prevStep = 'c*Mtc_BP1_trans_*.mat';
    case 'rereference'
        prevStep = 'M*Mtc_BP1_trans_*.mat';
    case 'TF_power'
        prevStep = 'tf*Mtc_BP1_trans_*.mat';
    case 'TF_phase'
        prevStep = 'tph*Mtc_BP1_trans_*.mat';
    case 'TF_rescale'
        prevStep = 'r*Mtc_BP1_trans_*.mat';
    case 'filter'
        prevStep = 'fb*Mtc_BP1_trans_*.mat';
    case 'secondfilter'
        prevStep = 'ffb*Mtc_BP1_trans_*.mat';
    case 'baseline'
        prevStep = 'b*Mtc_BP1_trans_*.mat';
    case 'average'
        prevStep = 'm*Mtc_BP1_trans_*.mat';
    case 'quickaverage'
        prevStep = 'quick/m*Mtc_BP1_trans_*.mat';
    case 'weight'
        prevStep = 'w*Mtc_BP1_trans_*.mat';
    case 'combineplanar'
        prevStep = 'p*Mtc_BP1_trans_*.mat';
    case 'artefact'
        prevStep = 'a*Mtc_BP1_trans_*.mat';
    case 'blink'
        prevStep = 'clean*Mtc_BP1_trans_*.mat';
    case 'image'
        prevStep = 'trial*Mtc_BP1_trans_*.img';
    case 'smooth'
        prevStep = 'sm*Mtc_BP1_trans_*.img';
    case 'firstlevel'
        prevStep = 't*Mtc_BP1_trans_*.img';
...
switch step
    ...
    case 'definetrials_tec' % uses modification of jp's trigger extraction function + includes ability to block out response triggers
        ...
        for s=1:size(subjects,1)
            ...
            for f=1:length(files)
                ...
                % get trials
                if length(p.triggers)< length(p.conditions)
                    cfg.triggers = p.triggers(1:3);
                    conditions = p.conditions(1:6);
                else
                ...
                for c = 1:length(trials.trl)
                    ...
                    elseif events(c).value == 2
                        offset_trials.trl(c,:) = trials.trl(c,:) + ([320 320 0]/1000)*cfg.fs;
                    elseif events(c).value == 3
                        offset_trials.trl(c,:) = trials.trl(c,:) + ([400 400 0]/1000)*cfg.fs;
                    else
                        error('Something went wrong, there should only be three event types')
                    end
                end
                
                trials.trl = zeros(2*length(offset_trials.trl),3);
                trials.trl(1:2:end-1,:) = onset_trials.trl;
                trials.trl(2:2:end,:) = offset_trials.trl;
                trials.labels = cell(2*length(offset_trials.trl),1);
                trials.labels(1:2:end-1,:) = onset_trials.labels;
                trials.labels(2:2:end,:) = offset_trials.labels;
    ...
    case 'definetrials_noham' % uses modification of jp's trigger extraction function + includes ability to block out response triggers
        ...
        for s=1:size(subjects,1)
            ...
            % First copy data if does not exist
            if ~exist([pathstem subjects '/tc_BP1_trans_' subjects '.mat'])
                if strcmp(subjects,'meg15_0208')
                    S.D = [maxfilteredpathstem '/tc_BP1_trans_meg15_208.mat']; %Typo in file creation
                elseif strcmp(subjects,'meg15_0521')
                    S.D = [maxfilteredpathstem '/tc_BP2_trans_' subjects '.mat']; %Different prefix for this subject
                else
                    S.D = [maxfilteredpathstem '/tc_BP1_trans_' subjects '.mat'];
                end
                S.outfile = [pathstem subjects '/tc_BP1_trans_' subjects '.mat'];
            ...
            for f=1:length(files)
                ...
                load(files(f).name);
                c = struct2cell(D.trials.events);
                sti.all = cell2mat(c(4,find(strcmp(c(1,:,1), 'STI101_up_new')),1))';
                sti.trigs = cell2mat(c(2,find(strcmp(c(1,:,1), 'STI101_up_new')),1))';
                sti.times = cell2mat(c(4,find(strcmp(c(1,:,1), 'STI101_up_new')),1))';
        ...
        for s=1:size(subjects,1)
            ...
            for f=1:length(files)
                ...
                if length(S.outfilestem) > 2
                    S.outfile = [S.outfilestem '/run_' files(f).name(1:end-4)];
                end
    ...
    case 'merge_recoded' % merge data with runname appended (e.g. for SD study, where each standard was done in a separate block)
        ...
        for s=1:size(subjects,1)
            ...
            blockname_starts = cell(size(p.blocksout{subjcnt}));
            for i = 1:length(p.blocksout{subjcnt})
                blockname_starts{i} = p.blocksout{subjcnt}{i}(1:p.filestring_length);
            end
            blocks_present = unique(blockname_starts);
            
            if length(blocks_present) < length(p.blocksout{subjcnt})
                S.recode = 'same';
                for i = 1:length(blocks_present)
                    try
                        clear these_files
                    catch
                    end
                    for j = 1:length(files)
                        if strfind(files(j).name,blocks_present{i})
                            if exist('these_files','var')
                                these_files(end+1) = files(j);
                            else
                                these_files = files(j);
                            end
                        end
                    ...
                    flag = 0;
                    mergePrimary = repmat(' ',1,50); % this is to ensure that the first file in list has the sensor locations etc. (because otherwise that information won't be retained in merged file)
                    mergeSecondary = repmat(' ',length(these_files)-1,50);
                    count = 0;
                    for f=1:length(these_files)
                        
                        
                        fprintf([ '\n\nProcessing ' these_files(f).name '...\n\n' ]);
                        
                        D = spm_eeg_load(these_files(f).name);
                        if ~isempty(D.fiducials) && flag == 0
                            flag = 1;
                            mergePrimary(1,1:length(these_files(f).name)) = files(f).name;
                        else
                            count = count + 1;
                            mergeSecondary(count,1:length(these_files(f).name)) = files(f).name;
                        end
                ...
                files = struct('name',new_files);
            end
            
            %Then take single runs and merge with filename prepended
            ...
            if isempty(files) && ~isempty(firstzero)
                ...
                for f=1:length(files)
                    ...
                    if strcmp(S.D.transformtype,'time')
                        ...
                        fix_resume_spm_eeg_average_TF(S);
                    end
